...
export default function Navbar() {
  ...
  return (
    <header className="relative border-b border-slate-200 bg-white/95 backdrop-blur dark:border-slate-800 dark:bg-slate-950/95">
      <div className="mx-auto grid max-w-6xl grid-cols-[1fr_auto] items-center px-6 py-4 md:grid-cols-[1fr_auto_1fr]">
        <Logo href="/" />
        <nav aria-label="Homepage navigation" className="hidden justify-self-center text-sm font-medium text-slate-700 dark:text-slate-200 md:flex">
        ...
        <div className="hidden items-center gap-1 justify-self-end text-sm font-medium text-slate-700 dark:text-slate-200 md:flex">
        ...
        <button type="button" className="justify-self-end rounded-lg p-2 text-slate-700 dark:text-slate-200 md:hidden" aria-label="Toggle navigation" aria-expanded={open} onClick={() => setOpen((value) => !value)}>{open ? <X /> : <Menu />}</button>
      </div>
      {open && (
