...
export default function Navbar() {
  ...
  return (
    <header className="relative border-b border-slate-200 bg-white/95 backdrop-blur dark:border-slate-800 dark:bg-slate-950/95">
        ...
        {isLoading ? <div className="h-10 w-48 animate-pulse rounded-xl bg-slate-200 dark:bg-slate-800" /> : (
          <>
            <nav aria-label="Primary navigation" className="hidden items-center gap-1 text-sm font-medium text-slate-700 dark:text-slate-200 md:flex">
              {links.map((link) => <Link onClick={close} key={link.href} href={link.href} aria-current={pathname === link.href ? "page" : undefined} className={`rounded-xl px-3 py-2 transition ${pathname === link.href ? "bg-indigo-50 text-indigo-700 dark:bg-slate-800 dark:text-white" : "hover:bg-slate-100 dark:hover:bg-slate-800"}`}>{link.label}</Link>)}
      ...
      {!isLoading && open && <nav aria-label="Mobile navigation" className="absolute inset-x-0 z-50 border-t border-slate-200 bg-white p-4 shadow-xl dark:border-slate-800 dark:bg-slate-950 md:hidden">{links.map((link) => <Link onClick={close} key={link.href} href={link.href} className="block rounded-lg px-3 py-3 text-sm font-semibold hover:bg-slate-100 dark:hover:bg-slate-800">{link.label}</Link>)}{isAuthenticated && <button onClick={() => { close(); logout(); router.replace("/login"); }} className="block w-full rounded-lg px-3 py-3 text-left text-sm font-semibold text-rose-600">Logout</button>}<div className="px-3 pt-2"><ThemeToggle /></div></nav>}
    </header>
  );
}
