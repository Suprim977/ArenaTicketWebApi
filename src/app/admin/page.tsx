...
export default function AdminHomePage() {
  ...
  const users = useQuery({ queryKey: ["admin-users"], queryFn: dashboardApi.getAdminUsers });
  ...
  const loading = events.isLoading || users.isLoading || bookings.isLoading || payments.isLoading;
  ...
  const revenue = paymentRows.filter((payment) => payment.status === "completed").reduce((sum, payment) => sum + payment.amount, 0);
  const ticketsSold = bookingRows.filter((booking) => booking.status === "confirmed").reduce((sum, booking) => sum + booking.quantity, 0);
  ...
  return (
    <section className="space-y-8">
      ...
      {(events.isError || users.isError || bookings.isError || payments.isError) && <p className="rounded-xl bg-amber-50 p-4 text-sm text-amber-700">Some management data could not be loaded. Available totals are shown below.</p>}
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <StatCard label="Total Events" value={String(eventRows.length)} helper="All event records" />
        <StatCard label="Total Users" value={String(users.data?.length ?? 0)} helper="Registered accounts" />
        <StatCard label="Total Bookings" value={String(bookingRows.length)} helper="All reservations" />
        <StatCard label="Tickets Sold" value={String(ticketsSold)} helper="Confirmed tickets" />
        <StatCard label="Total Revenue" value={`Rs ${revenue.toLocaleString("en-NP")}`} helper="Completed payments" />
      </div>
